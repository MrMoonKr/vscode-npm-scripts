...
export class NpmScriptsNodeProvider implements TreeDataProvider<ScriptTreeItem | WorkspaceTreeItem> {

    private readonly _onDidChangeTreeData: ScriptEventEmitter = new EventEmitter();
    public readonly onDidChangeTreeData: Event<MaybeScript> = this._onDidChangeTreeData.event;
     ...
     */
    private renderMultipleWorkspaces( resolve: Function, folders: readonly WorkspaceFolder[] ): void {
     ...
     * @memberof ScriptNodeProvider
     */
    private renderSingleWorkspace( resolve: Function, packageJsonPath?: string ): void {
    ...
    private mkTreeItemsForWorkspace( folders: readonly WorkspaceFolder[] ): WorkspaceTreeItem[] {
     ...
     * @memberof ScriptNodeProvider
     */
    private mkTreeItemsFromPackageScripts( packageJsonPath: string ): ScriptTreeItem[] {

        const treeItems: ScriptTreeItem[] = [];
        const packageJson = JSON.parse( fs.readFileSync( packageJsonPath, "utf-8" ) );
        const workspaceDir: string = path.dirname( packageJsonPath );

        const toScript = ( scriptName: string, scriptCommand: string ): ScriptTreeItem => {
